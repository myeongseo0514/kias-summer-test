...
두 개의 진자가 연결된 이중진자(Double Pendulum)의 운동을 브라우저에서 실시간으로 시뮬레이션하는 단일 HTML 페이지입니다. Lagrange 방정식과 RK4 수치 적분을 사용하여 물리 법칙을 정확히 따릅니다.

## 실행 방법

별도의 설치나 빌드가 필요 없습니다. `index.html` 파일을 브라우저에서 열면 됩니다.
...
```
.
├── index.html   # 시뮬레이션 (HTML + CSS + JavaScript)
├── AGENTS.md    # 프로젝트 요구사항
└── README.md    # 이 문서
```
...
이중진자는 카오스 시스템의 대표적인 예입니다. 초기 조건에 매우 민감하게 반응하여, 약간의 각도 차이만으로도 궤적이 크게 달라집니다. 질량, 길이, 중력을 바꿔보며 운동 패턴의 변화를 관찰해 보세요.
